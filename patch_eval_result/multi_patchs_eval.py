...
if __name__ == '__main__':
    ...
    patchs_path = "/root/Python_Program_Remote/MyAdvPatch/saved_patch/test6_old_loss_beta10"
    print("Loaded weights path: ", patchs_path)
    patchs = sorted(os.listdir(patchs_path))
    all_criterion = np.zeros((len(patchs), 7))
    index = 0
    for patch in patchs:
        adv_patch = Image.open(os.path.join(patchs_path, patch)).convert('RGB')
