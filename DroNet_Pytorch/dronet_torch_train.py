...
from DroNet_Pytorch.dronet_datasets import DronetDataset
...
def trainModel(model: dronet_torch.DronetTorch, 
    ...
    training_dataset = DronetDataset('/root/Python_Program_Remote/MyAdvPatch/datasets_png', 'training', augmentation=False, grayscale=False
                                        )
    validation_dataset = DronetDataset('/root/Python_Program_Remote/MyAdvPatch/datasets_png', 'validation', augmentation=False, grayscale=False)

    training_dataloader = torch.utils.data.DataLoader(training_dataset, batch_size=batch_size,
                                            shuffle=True, num_workers=10)
    validation_dataloader = torch.utils.data.DataLoader(validation_dataset, batch_size= int(batch_size / 2),
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (img, steer_true, coll_true) in tqdm(enumerate(training_dataloader), desc=f'Running epoch {epoch}', total=epoch_length):
        ...
        if epoch % steps_save == 0:
            ...
            weights_path = os.path.join('saved_models', 'test3_GRAY_torch', f'weights_{epoch:03d}.pth')
            torch.save(model.state_dict(), weights_path)
        # evaluate on validation set
        for batch_idx, (img, steer_true, coll_true) in tqdm(enumerate(validation_dataloader), desc=f'Running epoch {epoch}', total=len(validation_dataloader)):
...
if __name__ == "__main__":
    dronet = getModel((200, 200), 1, 1, None)
    print(dronet)
    trainModel(dronet, 200, 16, 1, 8)
